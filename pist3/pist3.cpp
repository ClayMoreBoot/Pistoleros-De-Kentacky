...
HANDLE memoria = CreateFileMapping(NULL, NULL, PAGE_READWRITE, 0, sizeof(int) * 4 + sizeof(char), "memoria1");
LPINT ref = (LPINT)MapViewOfFile(memoria, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(int) * 4 + sizeof(char)) + 250;
...
int main(int argc, char* argv[]) {
	...
	if (memoria == NULL) { exit(100); }
	if (ref == NULL) { exit(100); }

	if (libreria == NULL) { exit(100); }
	if (PIST_inicio == NULL) { exit(100); }
	if (PISTinicio == NULL) { exit(100); }
	if (PIST_nuevoPistolero == NULL) { exit(100); }
	if (PISTnuevoPistolero == NULL) { exit(100); }
	if (PIST_fin == NULL) { exit(100); }
	if (PIST_disparar == NULL) { exit(100); }
	if (PISTdisparar == NULL) { exit(100); }
	if (PIST_morirme == NULL) { exit(100); }
	if (PIST_vIctima == NULL) { exit(100); }
	...
	if ((evento = CreateEvent(NULL, TRUE, FALSE, NULL)) == NULL) { exit(100); }

	if ((semaforito1 = CreateSemaphore(NULL, 0, 2, "semaforo1")) == NULL) { exit(100); }
	if ((semaforito2 = CreateSemaphore(NULL, 0, 1, "semaforo2")) == NULL) { exit(100); }
	if ((semaforito3 = CreateSemaphore(NULL, 0, nPistoleros-1, "semaforo3")) == NULL) { exit(100); }
	...
	if ((PISTinicio(nPistoleros, rapidez, semilla)) == -1) { exit(100); }
	crearHijos(nPistoleros);

	
	if ((PIST_fin()) == -1) { exit(100); }
...
void crearHijos(int numeroHijos)
{
	...
	for (int b = 0; b < numeroHijos; b++) {
		if ((hijos[b] = CreateThread(NULL, 0, funcionHijo, LPVOID(hijoss[b]), 0, NULL)) == NULL) { exit(100); }
